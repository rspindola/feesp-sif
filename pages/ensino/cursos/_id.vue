<template>
  <div>
    <div>
      <div v-if="loading" class="title-loading skeleton p-2">
      ...
      <ValidationObserver ref="form">
        <form @submit.prevent="handleAddClass">
          <div class="box__new w-full">
            <h4 class="text-primary-dark">Criar Turma</h4>
            ...
            <div class="flex space-x-4 items-end">
              ...
              <div class="w-1/12">
                <Button text-label="Criar" class="px-6 py-2" />
              </div>
            </div>
          </div>
        </form>
      ...
      <div class="list">
        ...
        <div v-else>
          ...
          <div v-if="sortedItems.length === 0" class="text-center my-5">
            <p class="warning-no-items">Não possui turmas cadastradas</p>
          </div>
          <div
            ...
            v-else
            :key="i"
            :class="{ editing: item == editedClass }"
          ...
          >
            ...
            <div class="flex-1">
              ...
              <div class="edit">
                <div class="flex space-x-4">
                  <div class="w-1/12">
                    <CustomInput
                      v-model="editedClass.academicYear"
                      type="text"
                      placeholder="2021"
                      class="w-full"
                      rules="required"
            ...
            <div class="flex-none">
              <div class="view">
                <NuxtLink
                  :to="{
                    name: 'ensino-cursos-turma-id',
                    params: { id: item.id, course: setCourseTitle },
                  }"
                >
                  <outline-button
              ...
              <div class="edit">
                <Button
                  text-label="Salvar"
                  class="px-3 py-1"
                  @click.native="saveClassUpdate"
                />
                <button @click="editedClass = {}">cancelar</button>
              </div>
            </div>
          </div>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Tem certeza de que deseja excluir essa turma? Todos os seus
...
export default Vue.extend({
  ...
  async asyncData({ $auth, route, $axios }) {
    ...
    return { perm, courseDetails, courseId }
  },
  data() {
    return {
      formData: {
  ...
  computed: {
    ...mapState('courses', ['days', 'places', 'loading']),

    totalPages() {
      const pages = this.courseDetails.classes.length / this.pageSize
    ...
    sortedItems() {
      if (this.searchQuery) {
        return this.courseDetails.classes.filter((item) => {
          return this.searchQuery
            .toLowerCase()
            .split(' ')
            .every(
              (v) =>
                item.academicYear.toLowerCase().includes(v) ||
                item.room.toLowerCase().includes(v) ||
                item.obs.toLowerCase().includes(v) ||
                item.weekday.toLowerCase().includes(v) ||
                item.local.toLowerCase().includes(v)
            )
        })
      } else {
        return this.courseDetails.classes.filter((row, index) => {
    ...
    setCourseTitle() {
      return `${this.courseDetails.initials} –
          ${this.courseDetails.name}`
    },
  },
  async beforeCreate() {
    // await this.$store.dispatch('courses/show', { id: this.$route.params.id })
    await this.$store.commit('drawer/SET_TABS', this.tabs)
  },
  methods: {
    ...mapActions('classes', ['deleteClass', 'updateClass']),

    nextPage() {
      if (this.currentPage * this.pageSize < this.courseDetails.classes.length)
        this.currentPage++
    },
    prevPage() {
    ...
    setClassTitle(item) {
      return `${item.academicYear} - ${item.local} - ${item.weekday}
              ${item.time} - Sala ${item.room}`
    },
    setCourseParam(item) {
      return { initials: item.initials, name: item.name }
    },

    editData(course) {
      this.editedClass = course
    },
    ...
    async handleAddClass() {
      ...
      await this.$axios
        ...
        .then((response) => {
          ...
          this.courseDetails.classes.push(newCourse)
          this.formData = {}
          this.$swal({
            icon: 'success',
            title: 'Sucesso.',
        ...
        .catch((e) => {
          this.$swal({
            icon: 'error',
            title: 'Erro ao tentar cadastrar turma',
            toast: true,
            position: 'top-end',
            showConfirmButton: false,
            showCloseButton: true,
    ...
    async saveClassUpdate() {
      ...
      try {
        await this.updateClass(this.editedClass).then(() => {
          this.$swal({
            icon: 'success',
            title: 'Sucesso.',
    ...
    async confirmDelete() {
      const { id } = this.deletedClass

      try {
        const index = this.courseDetails.classes.indexOf(this.deletedClass)

        await this.deleteClass(id)

        if (index > -1) {
          await this.courseDetails.classes.splice(index, 1)
        }
        ...
        return this.courseDetails.classes
      } catch (error) {
        this.$swal({
          icon: 'error',
          title: error,
